    ...
    class Runner < ParallelTests::Test::Runner
      ...
      class << self
        def run_tests(test_files, process_number, num_processes, options)
          ...
          cmd = [xvfb_session_cmd(process_number), exe, options[:test_options],
                 (rspec_2_color if version == 2), spec_opts, *test_files].compact.join(" ")
        ...
        def xvfb_session_cmd(process_number)
          "xvfb-run -n #{process_number}"
        end
